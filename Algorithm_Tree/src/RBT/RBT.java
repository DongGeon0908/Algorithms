...
public class RBT<Key extends Comparable<Key>, Value> {
    ...
    ***************************************************************************/
    ...
    public int depth(Node k) {
        return depth1(k);
    }
    private int depth1(Node x) {
        if (x == null) {
        	return -1;
        }
        else if(x.left == null) {
	...
	private Node search1(Node node, Key k) {
		...
		if (cmp < 0) {
		    cc ++;
			return search1(node.left, k);
		} else if (cmp > 0) {
			cc++;
			return search1(node.right, k);
		} else {
